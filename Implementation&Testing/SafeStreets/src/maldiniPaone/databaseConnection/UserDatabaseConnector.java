 ...
 * {@link ConnectionPool}. This class's methods are used to create users check
 * their credential , modify them and delete users. All methods uses prepared
 * statements to avoid possibility of SQL injections. Results are encapsulated
 * in objects defined in {@link maldiniPaone.utilities}
 **/
public class UserDatabaseConnector {
	...
	/**
	 * Adds citizen to the database , gets a connection to the database from the
	 * connection pool and executes the insertion
	 * 
	 ...
	 * @return boolean: true if the creation is successful, false if there is
	 *         already a user with the given user name or email
	...
	protected static boolean AddCitizen(String username, String password, String email)
		...
		try {
			...
			// execute query
			ps.executeUpdate();
			// if query fails for duplicate user name or email exception is thrown
			res = true; // if code reaches this line the insert is successful
			// close statement
			ps.close();
			// release connection
	...
	/**
	 * Adds municipality to the database, gets a connection to the database from the
	 * connection pool and executes the insertion. Use this method when municipality
	 * is created by another municipality
	 * 
	 ...
	 * @param creator      :the user name of the user who asked the creation of the
	 *                     municipality
	 * @param cityName     : the name of the city hall where the municipality works
	 * @param cityProvince : the name of the city hall's province where the
	 *                     municipality works
	 * @return boolean: true if the creation is successful, false if there is
	 *         already a user with the given user name or email
	 * @throws DatabaseNotFoundException the connection to the database could not be
	 *                                   instantiated
	 **/
	protected static boolean addMunicipalityByMunicipality(String username, String password, String email,
			String creator, String cityName, String cityProvince) throws DatabaseNotFoundException {
		boolean res = false;
		Connection c = null;
		PreparedStatement ps = null;
		try {
	...
	/**
	 * Adds municipality to the database , gets a connection to the database from
	 * the connection pool and executes the insertion. Use this method when the
	 * creation is done by a system manager.
	 * 
	 * @param username     : the user name of the municipality to be added
	 * @param password     : the password of the municipality to be added
	 * @param email        : the email address of the municipality to be added
	 ...
	 *                                   instantiated
	 **/
	protected static boolean addMunicipalityByManager(String username, String password, String email, String cityName,
			String cityProvince) throws DatabaseNotFoundException {
		boolean res = false;
		Connection c = null;
		PreparedStatement ps = null;
		try {
			c = ConnectionPool.getInstance().getConnection();// get connection
			ps = c.prepareStatement("insert into municipality " // add municipality
					+ "(`username`,`password`,`email`,`cityhall_name`,`cityhall_province`) " + "values (?,?,?,?,?)");
			// set the values in the prepared statements avoid SQL injection
			ps.setString(1, username);
			ps.setString(2, password);
			ps.setString(3, email);
	...
	/**
	 * Adds authority to the database, gets a connection to the database from the
	 * connection pool and executes the insertion.
	 * 
	 ...
	 * @param creator    :the user name of the municipality who asked the creation
	 *                   of the authority
	...
	protected static boolean addAuthority(String username, String password, String email, String creator,
			Integer districtId) throws DatabaseNotFoundException {
		boolean res = false;
		Connection c = null;
		PreparedStatement ps = null;
		try {
			...
			ps = c.prepareStatement("insert into authority " // add authority
					+ "(`username`,`password`,`email`,`employee`,`district_id`)" + " values (?,?,?,?,?)");
			// set the values in the prepared statements avoid SQL injection
			ps.setString(1, username);
			ps.setString(2, password);
			ps.setString(3, email);
	...
	/**
	 * Adds system manager to the database, gets a connection to the database from
	 * the connection pool and executes the insertion.
	 * 
	...
	protected static boolean addManager(String username, String password, String email, String venueName)
		...
		try {
			...
			ps = c.prepareStatement("insert into manager "// add manager
					+ "(`username`,`password`,`email`,`venue`)" + " values (?,?,?,?)");
			// set the values in the prepared statements avoid SQL injection
			ps.setString(1, username);
			ps.setString(2, password);
			ps.setString(3, email);
	...
	/**
	 * Adds city hall to the database , gets a connection to the database from the
	 * connection pool and executes the insertion.
	 * 
	 ...
	 * @param location :the user name of the user who asked the creation of the
	 *                 municipality
	 * @return boolean: true if the creation is successful, false if there is
	 *         already a city hall with the same name and province
	 * @throws DatabaseNotFoundException the connection to the database could not be
	 *                                   instantiated
	 **/
	protected static boolean addCityhall(String name, String province, String region, Location location)
		...
		try {
			...
			ps = c.prepareStatement("insert into cityhall " // add cityhall
					+ "(`cityhall_name`,`cityhall_province`,`region`,`latitude`,`longitude`) " + "values (?,?,?,?,?)");
			// set the values in the prepared statements avoid sql injection
			ps.setString(1, name);
	...
	/**
	 * Adds district to the database , gets a connection to the database from the
	 * connection pool and executes the insertion.
	 * 
	 * @param name                : the name of the city hall in which the district
	 *                            is located
	 * @param province            : the name of the province in which the district
	 *                            is located
	 * @param locationTopLeft     : the coordinates of the top-left corner of the
	 *                            district area
	 * @param locationBottomRight : the coordinates of the bottom-right corner of
	 *                            the district area
	 * @return Integer: returns the index of the newly created district or -1 if the
	 *         creation is not successful
	 * @throws DatabaseNotFoundException the connection to the database could not be
	 *                                   instantiated
	 **/
	protected static Integer addDistrict(String name, String province, Location locationTopLeft,
			Location locationBottomRight) throws DatabaseNotFoundException {
		Connection c = null;
		Integer res = -1;
		PreparedStatement ps = null;
		try {
			...
			ps = c.prepareStatement("insert into district " // add district
					...
					+ "values (?,?,?,?,?,?)", PreparedStatement.RETURN_GENERATED_KEYS);// ask the statement to return
																						// the generated keys
			// set the values in the prepared statements avoid sql injection
			ps.setString(1, name);
			ps.setString(2, province);
			...
			// get the list of indexes created by the query, so the index of the new
			// district
	...
	protected static CityHall getCityHall(String username) throws DatabaseNotFoundException {
		...
		try {
			...
			ps = c.prepareStatement(
					"select ch.cityhall_name, ch.cityhall_province ,ch.region" + ", ch.longitude , ch.latitude"
					// get city hall
	...
	// ================================================================================
	/**
	 * This method checks if a user credentials are correct and at the same time it
	 * returns its user type
	 * 
	 * @param username : the user name of the user to be checked
	 * @param password : the password of the user to be checked
	 * @return UserType : returns if the user type of the user the credentials are
	 *         correct, null if no correspondence is found
	 * @throws DatabaseNotFoundException the connection to the database could not be
	 *                                   instantiated
	 **/
	...
	// ================================================================================
	/**
	 * Modifies the user name , password and email of a user using the given String
	 * parameters
	 * 
	 ...
	 * @return boolean : returns true if the modification is successful false
	 *         otherwise
	 * @throws DatabaseNotFoundException the connection to the database could not be
	 *                                   instantiated
	 **/
	protected static boolean modifyUser(String oldUsername, String oldPassword, UserType user, String newEmail,
			String newUsername, String newPassword) throws DatabaseNotFoundException {
		boolean res = false;
		Connection c = null;
		PreparedStatement ps = null;
		try {
			...
			ps = c.prepareStatement("update " + user.toString() // update the given user type
					+ " set username=?, password= ?, email=? " + " where username=? and password=? ");
			// set the values in the prepared statements avoid SQL injection
	...
	/**
	 * Modifies the user name and password of a user using the given String
	 * parameters
	 * 
	 * @param oldUsername : the user name of the user before being modified
	 * @param oldPassword : the password of the user before being modified
	 ...
	 *                                   instantiated
	 **/
	protected static boolean modifyUser(String oldUsername, String oldPassword, UserType user, String newUsername,
			String newPassword) throws DatabaseNotFoundException {
		boolean res = false;
		Connection c = null;
		PreparedStatement ps = null;
		try {
			...
			ps = c.prepareStatement("update " + user.toString() // update given user type
					+ " set username=?, password=? " + " where username=? and password=? ");
			// set the values in the prepared statements avoid SQL injection
			ps.setString(1, newUsername);
			ps.setString(2, newPassword);
	...
	/**
	 * Modifies the password of a user using the given String parameters
	 * 
	 * @param oldUsername : the user name of the user before being modified
	 * @param user        : the user type of the user to be modified
	 * @param newPassword : the new password of the user
	 ...
	 *                                   instantiated
	 **/
	protected static boolean forgotPasswordModification(String oldUsername, UserType user, String newPassword)
			throws DatabaseNotFoundException {
		boolean res = false;
		Connection c = null;
		PreparedStatement ps = null;
	...
	public static void main(String[] args) throws Exception {
		// UserDatabaseConnector.addCityhall("alserio", "como", "lombardia", new
		// Location(10.0f, 20.0f));
		// UserDatabaseConnector.addMunicipality("angelo", "paons", "pp", "alserio",
		// "como");
		// System.out.print(UserDatabaseConnector.addDistrict("alserio", "como" , new
		// Location(10.0f, 20.0f),new Location(10.0f, 20.0f)));
		// UserDatabaseConnector.AddCitizen("pietro", "maldini", "p@p");
		// UserDatabaseConnector.ModifyUser("pietro", "maldini", UserType.Citizen,
		// "pm390", "newone");
	}
}
