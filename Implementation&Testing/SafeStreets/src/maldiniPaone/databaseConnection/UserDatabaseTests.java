	...
	public void testAddUsers() {
		...
		try {
			for (int j = 0; j < cityHallsNames.length; ++j) {
				...
				UserDatabaseConnector.addCityhall(cityHallsNames[j], cityHallsProvinces[j], "lombardia", temp);
			}
			// TODO create districts
			int i;
			for (i = 0; i < usernames.length / 4; ++i)// add system managers
			{
				assertEquals(true, UserDatabaseConnector.addManager(usernames[i], passwords[i], 
						emails[i],venues[i % venues.length]));
				assertEquals(false, // trying to re-add user should result in failure
						UserDatabaseConnector.addManager(usernames[i], passwords[i], emails[i],
			...
			for (; i < usernames.length / 2; ++i) {
				assertEquals(true, UserDatabaseConnector.addMunicipalityByManager(usernames[i],
						passwords[i], emails[i],cityHallsNames[i % cityHallsNames.length], 
						cityHallsProvinces[i % cityHallsProvinces.length]));
				assertEquals(// trying to re-add the user should result in failure
						false,
						UserDatabaseConnector.addMunicipalityByManager(usernames[i], passwords[i], emails[i],
								cityHallsNames[i % cityHallsNames.length],
								cityHallsProvinces[i % cityHallsProvinces.length]));
